...
PRODOTTI = [
    ...
    },
    {
        ...
        "asins": {"Italia 🇮🇹": ("amazon.it", "B0CV4GMB3H"), "Germania 🇩🇪": ("amazon.de", "B0C43D5Q4S"), "Spagna 🇪🇸": ("amazon.es", "B0C43D5Q4S"), "Francia 🇫🇷": ("amazon.fr", "B0C43D5Q4S"), "Regno Unito 🇬🇧": ("amazon.co.uk", "B0C43D5Q4S")}
    },
    {
        "nome": "Xiaomi Redmi Note 13 5G",
        "asins": {"Italia 🇮🇹": ("amazon.it", "B0CQMH5DMB"), "Germania 🇩🇪": ("amazon.de", "B0B8R4J985"), "Spagna 🇪🇸": ("amazon.es", "B0B8R4J985"), "Francia 🇫🇷": ("amazon.fr", "B0B8R4J985"), "Regno Unito 🇬🇧": ("amazon.co.uk", "B0B8R4J985")}
    },
    {
        "nome": "Sony WH-1000XM5 Cuffie BT",
        "asins": {"Italia 🇮🇹": ("amazon.it", "B09Y2MYL5C"), "Germania 🇩🇪": ("amazon.de", "B09JR837S7"), "Spagna 🇪🇸": ("amazon.es", "B09JR837S7"), "Francia 🇫🇷": ("amazon.fr", "B09JR837S7"), "Regno Unito 🇬🇧": ("amazon.co.uk", "B09JR837S7")}
    },
    {
...
PERCENTUALE_MINIMA_SCONTO = 80.0  # Alert solo per sconti >= 80%
...
class SimpleHTTPRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        self.end_headers()
        self.wfile.write(b"Bot Amazon Europe Active!")
...
def invia_stato_discord(messaggio, tipo="info"):
    """Invia notifiche di stato (Inizio scansione o Errore) su Discord"""
    colore = 3447003 if tipo == "info" else 15158332  # Blu per info, Rosso per errore
    titolo = "🔍 Avvio Scansione Prodotti" if tipo == "info" else "⚠️ Problema Rilevato nel Bot!"
        ...
        "embeds": [
            {
                ...
                "color": colore,
                "footer": {"text": "Amazon Europe Tracker • Status Monitor"},
                "timestamp": time.strftime("%Y-%m-%dT%H:%M:%SZ")
            }
        ]
    }
    try:
        ...
        print(f"❌ Errore invio messaggio di stato su Discord: {e}", flush=True)
...
def controlla_prezzi():
    while True:
        ...
        print(f"\n--- Inizio ciclo di controllo Auto-Fallback ({ora_attuale}) ---", flush=True)
        
        # 1. NOTIFICA SU DISCORD: INIZIO SCANSIONE
        invia_stato_discord(f"🚀 *Il bot ha avviato un nuovo ciclo di scansione su 45 prodotti europei* ({ora_attuale}).", tipo="info")
        ...
        try:
            for prod in PRODOTTI:
                for paese, (domain, asin) in prod["asins"].items():
                    ...
                    try:
                        ...
                        # CASO 1: La pagina ASIN esiste (Status 200)
                        ...
                        # CASO 2: La pagina dà 404 -> AUTO-CORREZIONE VIA SEARCH
                        elif response.status_code == 404:
                            print(f"   🔄 [{paese}] 404 rilevato! Avvio ricerca automatica di fallback...", flush=True)
                            search_url = f"https://www.{domain}/s?k={quote(prod['nome'])}"
                            ...
                            if search_resp.status_code == 200:
                                ...
                                prezzo_attuale, prezzo_listino, sconto_percentuale = estrai_dati_prezzo(search_soup)
                                
                                if prezzo_attuale:
                                    print(f"   ✅ [{paese}] Recuperato via ricerca! Prezzo: {prezzo_attuale:.2f} € | Sconto: {sconto_percentuale:.1f}%", flush=True)
                                    if sconto_percentuale >= PERCENTUALE_MINIMA_SCONTO:
                                        invia_notifica_discord(prod['nome'], paese, prezzo_attuale, prezzo_listino, sconto_percentuale, search_url)
                                else:
                                    print(f"   ℹ️ [{paese}] Pagina di ricerca aperta, nessun prezzo scontato.", flush=True)
                            else:
                                print(f"   ⚪ [{paese}] Ricerca fallita.", flush=True)

                        else:
                            print(f"   ⚠️ [{paese}] Risposta server: {response.status_code}", flush=True)
            ...
            # 2. NOTIFICA SU DISCORD: ERRORE GRAVE NEL BOT
            print(f"💥 Errore critico nel ciclo principale: {e_critico}", flush=True)
            invia_stato_discord(f"⚠️ *Si è verificato un errore ed il bot si è interrotto!*\n`Dettaglio errore: {e_critico}`\nTentativo di riavvio in corso...", tipo="errore")
            time.sleep(60)

if __name__ == "__main__":
    print("🚀 Bot Amazon Europe Auto-Fallback 45 Products Avviato!", flush=True)
